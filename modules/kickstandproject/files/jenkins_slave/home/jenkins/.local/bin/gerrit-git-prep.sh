...
then
    ...
    echo "Triggered by: http://$SITE/$CHANGE_NUMBER"
fi
...
function merge_change {
    ...
    until git fetch http://$SITE/$PROJECT $REFSPEC
    do
        COUNT=$(($COUNT + 1))
        logger -p user.warning -t 'gerrit-git-prep' FAILED: git fetch http://$SITE/$PROJECT $REFSPEC COUNT: $COUNT
...
if [[ ! -e .git ]]
then
    git clone http://$SITE/$GERRIT_PROJECT .
fi
